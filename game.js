/** 
This asteroid game was made with pure javascript. 
it was an exercise of vector math and Euler's integration of motion.
...
class Vec {
  ...
  normalise() {
    let mag = this.mag();
    return new Vec(
      this.x !== 0 ? this.x / mag : 0,
      this.y !== 0 ? this.y / mag : 0
    );
  }
  limit(max) {
    const mag = this.mag();
    if (mag > max) {
      let { x, y } = this.normalise(mag).mult(max);
      this.x = x;
      this.y = y;
    }
    return new Vec(this.x, this.y);
...
async function updateScoreBoard() {
  ...
  let scoreData = new Array(10).fill(null).map((s, i, a) => {
    return { name: names[i], score: ~~(Math.random() * 9999) };
  });
  let sortedScoreData = scoreData.sort((a, b) => b.score - a.score);
  if (sortedScoreData && sortedScoreData.length) {
    let html = "";
...
let scene = {
  ...
  game() {
    ...
    setTimeout(() => {
      ...
      guiDom.style.animation =
        "fadeIn 500ms forwards cubic-bezier(0.4, 0, 0.2, 1)";
      Input.listen();
    }, 800);
    heartManager();
  },
...
class DamagePoint {
  ...
  draw() {
    ...
    CTX.font = `${this.size}px Theo Van Doesburg`;
...
class Ship {
  ...
  draw() {
    ...
    CTX.beginPath();
    CTX.moveTo(
      g[0][0] * this.scale + this.pos.x,
      g[0][1] * this.scale + this.pos.y
    );
    CTX.lineTo(
      g[1][0] * this.scale + this.pos.x,
      g[1][1] * this.scale + this.pos.y
    );
    CTX.lineTo(
      g[2][0] * this.scale + this.pos.x,
      g[2][1] * this.scale + this.pos.y
    );
    CTX.closePath();
    CTX.fill();
    CTX.restore();
...
function create(args) {
  let { type, qty, pos, rad, color, nature, dmg } = args;
  switch (type) {
    case "asteroid":
      for (let i = 0; i < qty; i++) {
        ...
        if (rad) {
          a = new Asteroid(
            new Vec(pos.x, pos.y).add(randOff),
            rad,
            new Vec(x, y)
          );
        } else {
    ...
    case "thrust":
      for (let i = 0; i < qty; i++) {
        ...
        let randAcc = new Vec(
          helpers.random(acos(a) - 2, acos(a) + 2),
          asin(a)
        );
        ...
        particleList.push(
          new ThrustParticle(new Vec(randX, randY), randAcc, r)
        );
      }
      break;
    case "explosion":
      for (let i = 0; i < ~~helpers.random(10, 20); i++) {
        ...
        if (color) {
          explosionList.push(
            new Explosion(new Vec(x, y), new Vec(ax, ay), color, nature)
          );
        } else {
          explosionList.push(
            new Explosion(new Vec(x, y), new Vec(ax, ay), null, nature)
          );
        }
      }
      break;
...
replayDom.addEventListener("click", (e) => {
  // Replay Button
  e.preventDefault();
  state.score = 0;
  state.scoreId = "";
  gameOverDom.style.animation =
    "fadeOut 500ms forwards cubic-bezier(0.4, 0, 0.2, 1)";
  setTimeout(() => {
    scene.game();
...
submit.addEventListener("click", (e) => {
  e.preventDefault();
  alert("The scoreboard is currently disabled");
  // sendScore();
